...
| `dingtalk_authorize_signup` | 允许自动注册 | ✅ **推荐勾选**（SSO 直接登录） |
...
| `dingtalk_authorize_signup` | Boolean | `true` | ⭐️ **重要** 允许钉钉用户自动注册（独立于全局设置，**推荐启用**） |
...
```
用户 → 点击"使用钉钉登录" → 钉钉 SSO 认证 → ✅ 直接登录成功
```

- ✅ 新用户自动创建并激活
- ✅ 虚拟邮箱用户自动激活
- ✅ 真实邮箱用户也直接登录（SSO 已验证身份）
- ✅ 无需跳转到注册页面
- ✅ 无需手动点击注册按钮
- ✅ 无需邮箱验证
...
所有通过钉钉 SSO 的邮箱（包括虚拟邮箱）都被标记为已验证，因为：
1. 钉钉 SSO 已经验证了用户身份
2. 可以信任钉钉提供的用户信息
3. 提供更好的用户体验（一键登录）
